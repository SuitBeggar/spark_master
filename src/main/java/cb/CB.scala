...
object CB {
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setAppName("recommendersystemCB")

    val sc = new SparkContext(conf)

    //val rdd = sc.textFile("hdfs://master:9000/cf/input/cf.data")
    val rdd = sc.textFile("hdfs://master:9000/recommendersystem/cb/input/cb_train.data")
   ...
   var step2 = step1.flatMap(x=>{
    ...
    }).saveAsTextFile("hdfs://master:9000/recommendersystem/cb/output_spark")
